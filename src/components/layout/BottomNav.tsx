...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-100 safe-bottom z-50">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link
              ...
              href={tab.href}
              className={cn(
                'flex flex-col items-center gap-0.5 px-4 py-1 rounded-xl transition-colors min-w-[60px]',
                active ? 'text-emerald-600' : 'text-zinc-400'
              )}
            >
              {tab.icon(active)}
              <span className={cn('text-[11px] font-medium', active ? 'text-emerald-600' : 'text-zinc-400')}>
                {tab.label}
              </span>
            </Link>
          )
